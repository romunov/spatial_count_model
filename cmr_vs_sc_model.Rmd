...
```{r prepare_data_scr}
### CMR model
# augment data - add M number of "phantom" individuals
M <- 123
...
```{r}
# See page
mark.pois.N <- SCR0pois(y = augmented.y, X = X, M = M, xl = xl, xu = xu, yl = yl, yu = yu, delta = c(1, 0.1, 2), 
                       niter = 6000)

mark.pois.N.mcmc <- mcmc(mark.pois.N)
plot(window(mark.pois.N.mcmc, start = start))
summary(window(mark.pois.N.mcmc, start = start))
```

### Model with binomial encounter process
```{r capture_mark_recapture_model}
# run the model
# TODO: ta model mogoče ne dela, ker imam za posamezno past več kot 1 ulov, model pa 
# predpostavlja, da je osebek lahko zaznan samo enkrat v pasti (j) in časovnem intervalu (k)
# za več poglej stran 455
mark.N <- SCR0binom.cl(y = augmented.y, X = X, M = M, xl = xl, xu = xu, yl = yl, yu = yu, K = K, delta = c(1, 0.1, 2), 
                       niter = 6000)

mark.N.mcmc <- mcmc(mark.N)
summary(window(mark.N.mcmc, start = start))
plot(window(mark.N.mcmc, start = start))
```

## Spatial count model
```{r spatial_count_model}
### SC model
...
summary(window(un.N.mcmc, 1000))
plot(window(un.N.mcmc, 1000))
autocorr(un.N.mcmc)
