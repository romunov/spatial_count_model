...
```{r joses_nimble_simulation, eval = FALSE}
...
paraFunction <- function(x, pass = 100, data.model = c("poisson", "binomial"),
  ...
  save(pois, binom, unmarked, x, N, sigma, lam0, J, K, A, data.model, 
...
# If scrapeModelResults gives warning
#
...
#
# which is caused by mcmc(x) because there's no data, only error

scrapeModelResults <- function(f) {
  load(f)
...
suppressWarnings(
  mr <- sapply(X = list.files(pattern = "sim_"), FUN = scrapeModelResults, simplify = FALSE)
)
mr <- do.call("rbind", mr)
rownames(mr) <- NULL

levels(mr$data.model) <- c("data.model: binom", "data.model: poisson")
levels(mr$model) <- c("est: binom", "est: poisson", "est: unmarked")
```

```{r}
suppressWarnings(print(
  ggplot(mr, aes(x = as.factor(run), y = X50.)) +
  theme_bw() +
  xlab("Run") +
  ylab("Estimated number of individuals") +
  geom_pointrange(aes(ymin = X2.5., ymax = X97.5.)) +
  geom_point(aes(y = N), shape = "+") +
  facet_grid(model ~ data.model)
))
# ggsave("mr.pdf", width = 8, height = 8)
```
Points designated with a `+` are true population size whereas points (with credible intervals) are those from the estimation procedure.
I notice a few things on this plot:

- Data generated by the binomial model (first column) results in estimates less often than for Poisson model (right column).
- Binomial model has trouble fitting the data generated using Poisson (upper right) even though R tries to fit the model using different starting values 100 times.
- Binomial and Poisson models are constantly biased downward. The bias seems too regular so I suspect something fishy is going on. The only culprit I can think of is my data generation (still).
- Unmarked model appears to have gargantuan credible intervals (not useful for management decisions?) and most of the time exhibits downward bias.
