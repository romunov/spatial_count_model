...
```{r parallel_analysis}
# sum the data for each trap across all individuals for unmarked
n <- apply(y, MARGIN = c(2, 3), FUN = sum)
...
cs <- makeCluster(3)
delta <- c(1, 0.1, 2)
clusterExport(cl = cs, c("augmented.y", "M", "X", "xl", "xu", "yl", "yu", "delta", "niter", "SCR0pois", "n",
                         "binom.augmented.y", "SCR0binom.cl", "scrUN", "xlim", "ylim"))

sim <- parLapply(cl = cs, X = 1:3, 
                 fun = function(null, augmented.y, binom.augmented.y, n, Xcoord, M, xl, xu, yl, yu, delta, niter,
                                xlim, ylim, K) {
                     pois <- tryCatch(mark.pois.N <- SCR0pois(y = augmented.y, X = Xcoord, M = M, xl = xl, 
                                                              xu = xu, yl = yl, yu = yu, delta = delta, 
                                                              niter = niter), 
                                      error = function(e) e)
                     
                     binom <- tryCatch(SCR0binom.cl(y = binom.augmented.y, X = Xcoord, M = M, xl = xl,
                                                    xu = xu, yl = yl, yu = yu, K = K, 
                                                    delta = c(1, 0.1, 2), niter = niter),
                                       error = function(e) e)
                     
                     unmarked <- tryCatch(scrUN(n = n, X = Xcoord, M = M, niter = niter, xlims = xlim, ylims = ylim,
                                                inits = list(lam0 = 0.1, sigma = rnorm(1, mean = 5, sd = 0.1)), updateY = FALSE,
                                                tune = c(0.004, 0.09, 0.35)),
                                          error = function(e) e)
                     
                     return(list(pois = pois, binom = binom, unmarked = unmarked))
                 }, 
                 augmented.y = augmented.y, binom.augmented.y = binom.augmented.y, n = n, Xcoord = X, M = M, xl = xl, 
                 xu = xu, yl = yl, yu = yu, delta = c(1, 0.1, 2), xlim = xlim, ylim = ylim,
                 niter = niter, K = K)
```
